...
EXCECAO COHAPM JURIDICO: ha preset obrigatorio Salvador–BA (tabela geo_targeting_presets, meio=juridico). Default automatico; qualquer geo diferente e REJEITADA. La Felicità NÃO herda. Legal é Viver NÃO herda. Detectar meio por params.meio ou nome da campanha/conjunto (JURIDICO/JUR_ vs LAFELICITA/LF_).
$fato$,
  true,
  '2026-08-21'
where not exists (
...
fs.writeFileSync("supabase/migrations/20260821220000_geo_preset_juridico_salvador.sql", sql);
fs.writeFileSync("supabase/espelhos/20260821220000_geo_preset_juridico_salvador.sql", sql);
console.log("fixed quotes,", arr.length, "bairros");
