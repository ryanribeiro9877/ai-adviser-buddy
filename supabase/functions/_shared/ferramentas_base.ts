...
//
// efeito e setor entraram em 03/09/2026 e ficam AQUI, e nao so na tabela, porque a guarda de
// ato que eles habilitam nao pode existir apenas no caminho feliz: um turno degradado (leitura
// de agent_ferramentas falhou) e exatamente o turno em que o modelo mais improvisa. Um snapshot
// que perdesse o efeito faria a guarda desligar em silencio, que e a falha que o campo veio
// impedir. _prova_efeito_ferramentas.ts guarda a igualdade entre este arquivo e a tabela.
...
export type FerramentaBase = {
  ...
  superficies: string[];
  /**
   * 'escrita' = chamar produz estado duravel FORA da resposta do turno (card de aprovacao,
   * biblioteca da Meta, store da conversa, versao de perfil). 'leitura' = nao produz.
   *
   * A semantica e de FALHA ABERTA: ausente ou 'leitura' significa NAO VERIFICA. A guarda
   * generica de ato ("afirmou ato do setor X sem ter chamado ferramenta de escrita do dono de
   * X") so olha 'escrita', entao classificar de menos custa uma verificacao que nao acontece,
   * e classificar de mais devolve um turno correto acusando ato que de fato existiu.
   */
  efeito: "leitura" | "escrita";
  /** Setor dono, no MESMO vocabulario de public.agents.setor. A igualdade e o que torna o par
   *  (setor, efeito) computavel sem uma tabela de-para. */
  setor: string;
  /**
...
export const FERRAMENTAS_BASE: Record<string, FerramentaBase> = {
  ...
  custo_llm_periodo: {
    ...
    superficies: ["chat","job"],
    efeito: "leitura",
    setor: "Saude da plataforma e pendencias",
  },
  ...
  propose_action: {
    descricao:
      "Cria PEDIDO DE APROVACAO (ActionCard) para todo ato na conta Meta. NAO executa: o card fica PENDENTE, so um administrador aprova, e expira em 24h. SO use quando o gestor pedir o ato com verbo explicito (emitir, criar, subir, pausar, ativar, escalar, duplicar, renomear, alterar, vincular). PERGUNTA sem verbo de ato — inclusive 'antes da aprovacao' e 'o anuncio esta com o mesmo link' — se responde com get_aprovacoes, get_estrutura_conjuntos ou get_criativos_conteudo, NAO com esta tool. Desativar comentario do post Instagram NAO e action_type: nao emita card (o interruptor e no Instagram/Business Suite; pausar boost nao fecha comentario). Exige justificativa, metrica_sucesso e reversa. target_name e o nome ATUAL do objeto; quando o nome nao for unico, mande params.alvo_external_id com o id da Meta. EXCLUIR nao existe em nenhum nivel: para tirar do ar use pausar_*. Sem approval_id no retorno, o card NAO existe. Pedido de varios cards: varias chamadas nesta mesma resposta, uma por peca (minimo 3, teto 4 por bloco). Nao feche o bloco com um card so.",
