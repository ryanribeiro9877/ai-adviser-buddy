// supabase/functions/traffic-agent-job/index.ts (v4.13)
// v4.13 (31/08/2026) - get_whatsapp_da_pagina no especialista whatsapp_waba
//   (dropdown da Pagina para CTWA; distinto de get_waba_status).
  ...
  if (deepHit) {
    const planoMagro = overviewLean
      ? [
          ...
          { nome: "criativos", foco: FOCO_CRIATIVOS_OVERVIEW },
          pedeWaba
            ? { nome: "whatsapp_waba", foco: "get_waba_status com meio se o pedido for Juridico/La Felicita. get_whatsapp_da_pagina se for habilitar numero no conjunto. Separe WABA Cloud/ON_PREMISE (CONNECTED=de pe) de CTWA inventario (IN_ADS nao e de pe). Declare DISCONNECTED. Nao diga que so ha wa.me dos anuncios." }
...
- whatsapp_waba: inventario WhatsApp (get_waba_status + get_whatsapp_da_pagina): WABA Cloud/ON_PREMISE vs CTWA vs dropdown da Pagina; de_pe, meio JUR/LF/VISTTA; templates so se pedido
...
Deno.serve(async (req) => {
  ...
  if (String(body?.modo ?? "") === "drive_watch") {
    ...
    return json({ ok: true, modo: "drive_watch", versao: "job-v4.13",
