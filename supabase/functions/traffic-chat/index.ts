// supabase/functions/traffic-chat/index.ts (v28.85)
// v28.85 (01/09/2026) - CTWA: numero do conjunto em digitos (display so no texto) e
//   doutrina corrigida pela auditoria das 11:49 — casou_na_api=false significa numero
//   fora das WABAs da conta, entao a API recusa 1487246 em qualquer formato. Agente
//   passa a dizer o que falta (vinculo WABA) em vez de emitir card fadado a falhar.
...
const TOOLS = [
  ...
  { type: "function", function: { name: "get_whatsapp_da_pagina", description: "CHECAGEM obrigatoria antes de emitir conjunto CTWA: diz se o numero e ativo WhatsApp da conta (Pagina, WABAs do Business, conjuntos existentes). parecer.e_ativo_whatsapp_da_conta=true → EMITA criar_conjunto_a_partir_de com destination_type=WHATSAPP (Messenger OFF) e whatsapp_phone_number em DIGITOS. false → a Marketing API recusa 1487246 em qualquer formato (medido 01/09/2026); explique que falta vincular o numero a uma WABA no WhatsApp Manager e ofereca criar o conjunto no Gerenciador. Distinto de get_waba_status. Escrita = propose_action — Pipeboard create_adset e recusado em ler_pipeboard. COHAPM: nao misture numero Juridico em VISTTA.", parameters: { type: "object", properties: { numero: { type: "string", description: "Telefone do conjunto (com ou sem o 9 extra)." } } } } },
...
- CTWA SO CRIA COM NUMERO QUE E ATIVO DA CONTA (01/09/2026 v28.85, medido): auditoria dos cards VISTTA CONJ.1-4 as 11:49 UTC mostra os quatro formatos (display, 12 digitos, +E.164, 13 digitos) recusados com o MESMO 1487246. Os numeros 7199189-4229, 7199185-8107, 7199264-9576, 7199188-7731 nao aparecem em nenhuma WABA, ad_set ou waba_phone_numbers da conta. O Destino manual do Gerenciador lista numero ligado so a Pagina; a Marketing API exige o numero como ativo WhatsApp do Business. Entao: get_whatsapp_da_pagina com casou_na_api=true → EMITA. casou_na_api=false → NAO prometa o conjunto e NAO fique reexecutando card: diga que a API recusa esse numero enquanto ele nao estiver numa WABA do Business (WhatsApp Manager), e que a alternativa imediata e criar esse conjunto no Gerenciador. PROIBIDO inventar que "o numero nao existe" ou "nao esta no seletor da Pagina" — ele esta no seletor; o que falta e o vinculo WABA. PROIBIDO trocar por numero Juridico/La Felicita.
...
Chame get_whatsapp_da_pagina(numero=...) ANTES de emitir: parecer.e_ativo_whatsapp_da_conta=true → EMITA.
false → a Marketing API recusa 1487246 em todo formato; explique que falta vincular o numero a uma
WABA do Business (WhatsApp Manager) e ofereca criar esse conjunto no Gerenciador, sem reexecutar card.
