...
const TOOLS = [
  ...
  { type: "function", function: { name: "registrar_veredito_peca_em_revisao", description: "PORTA UNICA para o RESPONSAVEL fechar revisao de compliance em pecas_em_revisao. SO use quando o humano (ex.: Roberto) der o veredito explicitamente - NUNCA invente nem antecipe. Valores: liberado_como_esta (desliga bloqueia_uso; executor passa a aceitar), ajustar_peca ou nao_usar (mantem bloqueio). Exige drive_file_id, veredito e veredito_por (nome de quem decidiu). Nota opcional fica anexada ao motivo. Nao faca UPDATE a mao.", parameters: { type: "object", properties: { drive_file_id: { type: "string" }, veredito: { type: "string", enum: ["liberado_como_esta", "ajustar_peca", "nao_usar"] }, veredito_por: { type: "string", description: "Quem decidiu (ex.: Roberto). Obrigatorio." }, nota: { type: "string", description: "Opcional: condicao ou justificativa anexada ao motivo." } }, required: ["drive_file_id", "veredito", "veredito_por"] } } },
  ...
  try {
    switch (name) {
      ...
      case "registrar_veredito_peca_em_revisao": return await t_rpc("registrar_veredito_peca_em_revisao", {
        ...
        p_veredito_por: String(args?.veredito_por ?? ""),
...
- VEREDITO DE PECA EM REVISAO: a porta unica e registrar_veredito_peca_em_revisao. So chame quando
  o responsavel (ex.: Roberto) der o veredito com todas as letras. Nunca antecipe, nunca UPDATE a mao.
  liberado_como_esta desbloqueia; ajustar_peca e nao_usar mantem o impedimento.
