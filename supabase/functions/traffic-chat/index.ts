...
//   existe card de renomeacao de anuncio; faca no Gerenciador" — que era verdade sobre o
//   sistema, nao sobre a Meta: renomear e POST /{id} com `name` nos tres niveis. Entram
//   renomear_conjunto e renomear_criativo (execucao em meta-actions v5.60, espelho de nome em
//   ad_sets/ads). A descricao da tool passa a dizer que renomear existe nos tres niveis e que
//   excluir nao existe em nenhum — as duas confusoes que geraram trabalho manual hoje.
...
async function t_propose_action(companyId: string, convId: string, requestedBy: string, args: any, cards: CardInfo[]) {
  ...
  const summaryBase = ({ pausar_criativo: `Pausar o criativo "${alvo.name}"`,
    ...
    renomear_campanha: `Renomear a campanha "${alvo.name}" para "${String(params?.novo_nome ?? "").trim()}"`,
    renomear_conjunto: `Renomear o conjunto "${alvo.name}" para "${String(params?.novo_nome ?? "").trim()}"`,
    renomear_criativo: `Renomear o anuncio "${alvo.name}" para "${String(params?.novo_nome ?? "").trim()}"`,
    alterar_categoria_especial_campanha: (() => {
...
// A ORDEM em que as ferramentas entram no corpo da requisicao. As definicoes vieram para
// public.agent_ferramentas; a ordem ficou aqui porque leitura de tabela nao tem ordem estavel
// e requisicao que muda de forma a cada turno joga fora o cache de prefixo do provider.
// Chave que exista no registro e falte nesta lista entra depois, em ordem alfabetica.
const ORDEM_TOOLS = [
  "get_overview",
  "get_alerts",
  "get_recommendations",
...
// PROMPT DO GESTOR (AG-00). Tres camadas, e elas nao se misturam:
//
//   1. AQUI mora o que vale para o turno inteiro, independente de qual ferramenta e chamada:
//      identidade, hierarquia de prioridades, limites duros de ato, anti-alucinacao e escopo.
//   2. NA FERRAMENTA (public.agent_ferramentas.descricao) mora o que decide SE ela e chamada.
//   3. NA DOUTRINA DA FERRAMENTA (agent_ferramentas.doutrina) mora o que decide COMO ler o
//      retorno dela — e essa so entra no turno quando a ferramenta e efetivamente executada.
//
// 03/09/2026: este prompt tinha 45.685 chars porque as tres camadas estavam empilhadas aqui.
...
3. Proteger o custo: teto vigente, protecao de custo no conjunto, gasto sob controle.
...
ESTA FORA DO SEU ESCOPO e voce NAO comenta, analisa nem recomenda: relacao com bancos, roteamento de propostas, esteira interna, politica de credito, operacao de atendimento humano, margem por banco, processos internos, e plataformas alem da Meta (nao ha dado de nenhuma outra). Se perguntarem, responda que isso e tratado internamente pela empresa e siga para o que e trafego.

== PROTOCOLO OBRIGATORIO ANTES DE RESPONDER ==
1. PLANEJE: identifique o que a pergunta exige e QUAIS ferramentas trazem cada parte. Prefira chamar as necessarias na MESMA rodada.
2. COLETE: rode as ferramentas. Nunca responda de memoria sobre numeros - so o que a ferramenta devolveu nesta conversa vale como dado.
3. CONFIRA cada numero antes de escrever: (a) de qual ferramenta veio? (b) qual periodo exato? (c) a cobertura cobre esse periodo inteiro (dias_com_dado / cobertura_real / cobertura)? (d) o denominador e estavel ou esta em ingestao?
4. SEGMENTE antes de concluir tendencia: medias historicas escondem mudancas. Havendo serie por mes, leia o mes mais recente, nao a media.
5. RESPONDA com numero + fonte + ressalva. Se algo nao fecha, diga que nao fecha em vez de escolher a versao mais bonita.

== PROIBIDO: RESPOSTA SO DE INTENCAO (20/08/2026) ==
...
- Dinheiro acima de volume: contrato pago mandaria mais que CPL, mas essa fonte NAO existe no sistema (fora de escopo desde 28/07). Declare sempre que CPL e proxy e que a unidade economica nao e mensuravel aqui.
...
== CONHECIMENTO DE PLATAFORMA (resumo operacional; o detalhe esta na base acima) ==
${legal ? `CATEGORIA ESPECIAL "PRODUTOS E SERVICOS FINANCEIROS": obrigatoria quando a campanha for de credito/financiamento. Confirme o produto antes de aplicar. PROMESSA ENGANOSA em credito: nunca prometa aprovacao, taxa certa ou dinheiro imediato.` : `Categoria especial financeira e doutrina de credito NAO se aplicam por padrao nesta empresa. So marque categoria especial se leitura atual + razao regulatoria DESTA empresa comprovarem.`}
CBO vs ABO: CBO concentra entrega no conjunto que responde melhor; ABO da controle por publico e evita que um conjunto absorva tudo. Estrutura hibrida na mesma conta dificulta comparacao justa entre conjuntos.
EVENTO DE OTIMIZACAO: a Meta entrega para quem tem propensao a gerar o EVENTO otimizado. Sinal raso (formulario, clique) traz volume barato de quem preenche facil - e a causa mais comum de "lead barato que nao vira venda".
FADIGA: frequencia crescente com CTR caindo e custo por resultado subindo no mesmo publico. Antes de trocar criativo, verifique se a queda nao e de entrega, orcamento ou sazonalidade.
APRENDIZADO LIMITADO: conjunto que nao atinge o volume minimo de eventos na janela sai do aprendizado sem estabilizar e o custo oscila. Fracionar orcamento em muitos conjuntos e a causa tipica.
ATRIBUICAO: a janela padrao e 7 dias de clique e 1 de visualizacao. Janela maior infla o resultado aparente, menor subestima, e comparar periodos com janelas diferentes invalida a comparacao.
...
Alem delas: em pedido amplo, consulte o que for necessario e responda bloco a bloco na ordem pedida. Escreva de forma continua ate acabar; se a mensagem for cortada por tamanho, o sistema emenda sozinho - NAO pare voluntariamente, nao pergunte se pode continuar, nao encerre com "nao foi retornado nesta rodada" e nunca responda "nao consegui".
...
Deno.serve(async (req) => {
  ...
  // O registro de ferramentas sai na mesma largada do Roteador: a leitura corre junto com a
  // viagem do modelo de triagem e com a montagem do historico, entao nao soma parede. Falha
  // aberta esta dentro de carregarFerramentas — esta promessa nao rejeita.
  const promessaFerramentas = carregarFerramentas(supa);
  ...
  const ferramentasDoTurno = agentesDoTurno.length && delegacao
    ...
    : null;
  // O registro chegou junto com a delegacao (mesma janela de espera, custo zero de parede).
  // Estreitamento e recorte, nao definicao: sem agente escolhido, vale o conjunto inteiro.
  const catFerr = await promessaFerramentas;
  const toolsDoTurno = montarFerramentas(catFerr, "chat", ferramentasDoTurno, ORDEM_TOOLS);
