// supabase/functions/traffic-chat/index.ts (v28.83)
...
const TOOLS = [
  ...
  { type: "function", function: { name: "get_whatsapp_da_pagina", description: "LEITURA Graph/WABA/conjuntos para formatar o numero do conjunto CTWA. NAO e o seletor visual do Gerenciador: casou_na_api=false NAO autoriza recusar. Se parecer.pode_usar_no_conjunto=true (digitos validos), EMITA criar_conjunto_a_partir_de com destination_type=MESSAGING_MESSENGER_WHATSAPP e display_gerenciador (+55 71 9189-4229). Distinto de get_waba_status. Escrita = propose_action — Pipeboard create_adset e recusado em ler_pipeboard. COHAPM: nao misture numero Juridico em VISTTA.", parameters: { type: "object", properties: { numero: { type: "string", description: "Telefone do conjunto (com ou sem o 9 extra). Ausencia no inventario Graph nao recusa." } } } } },
...
- WHATSAPP NO CONJUNTO (01/09/2026 v28.83): copie o Gerenciador. Destino MANUAL = Messenger + WhatsApp (destination_type=MESSAGING_MESSENGER_WHATSAPP) e o numero no formato +55 71 9189-4229. NAO use destino automatico. get_whatsapp_da_pagina NAO e o seletor da Pagina: casou_na_api=false NAO significa "numero nao encontrado no seletor". PROIBIDO recusar CONJ.1–4 VISTTA, dizer "nao emitir ainda" ou pedir para habilitar numeros na Pagina. Pedido "vamos aos conjuntos" / campanha ja criada = EMITIR os cards (ou pedir reexecucao se ja houver card re_executavel). Prova 01/09: Ads Manager CONJ.1 Destino manual +55 71 9189-4229 na Pagina Cohapm. PROIBIDO numero Juridico em VISTTA. Se o card falhou 1487246, peca reexecutar — o executor manda Destino manual na primeira tentativa.
...
- CONJUNTO CTWA NAO RECUSA POR INVENTARIO GRAPH (01/09/2026 v28.83): se o gestor pediu os conjuntos (VISTTA / COHAPM_VISTTA_CONV_WA_AGO26 / "vamos aos conjuntos") com os numeros +55 71 9189-4229, 9185-8107, 9264-9576, 9188-7731, EMITA ou peca reexecutar. casou_na_api=false NAO e "nao encontrado no seletor da Pagina". PROIBIDO: "nao emitir ainda", "habilite os quatro numeros na Pagina", "nao foi possivel concluir a leitura = sem confirmacao = nao emito". Digitos validos = pode_usar_no_conjunto. Cards re_executavel da mesma campanha: peca reexecutar; se nao houver, EMITA criar_conjunto_a_partir_de.
...
Chame get_whatsapp_da_pagina(numero=...) so para formatar display_gerenciador — NAO e o seletor da Pagina.
casou_na_api=false NAO bloqueia. Se parecer.recusar=false e pode_usar_no_conjunto=true, EMITA.
PROIBIDO recusar VISTTA porque a lista Graph omitiu o numero. PROIBIDO pedir para habilitar numero na Pagina.
