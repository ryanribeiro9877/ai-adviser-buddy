// supabase/functions/traffic-chat/index.ts (v28.28)
...
async function t_propose_action(companyId: string, convId: string, requestedBy: string, args: any, cards: CardInfo[]) {
  ...
  const entityType = action === "alterar_orcamento"
    ? "budget"
    : (action === "ajustar_posicionamentos_do_conjunto" || action === "pausar_conjunto")
      ? "adset"
      : (isAd ? "ad" : "campaign");
  const summaryBase = ({ pausar_criativo: `Pausar o criativo "${alvo.name}"`,
    ...
    pausar_conjunto: `Pausar o conjunto "${alvo.name}" (status PAUSED; ativacao de conjunto/campanha continua manual no Gerenciador)`,
...
async function t_propose_criacao(companyId: string, convId: string, requestedBy: string, args: any, cards: CardInfo[], mcpKey: string) {
  ...
  if (action === "criar_campanha") {
    ...
    const summary = `Criar campanha "${nomeAlvoComposto}" (objetivo ${objetivo}, papel ${montado.partes.papel}) - nasce PAUSADA, categoria especial de credito obrigatoria — nome composto ESP-40/39`;
...
async function gravarCard(companyId: string, convId: string, requestedBy: string, action: string,
  ...
  return { ok: true, approval_id: ins.id, resumo: summary, expira_em: ins.expires_at,
    aviso: action === "criar_anuncio_a_partir_de"
      ? "Pedido PENDENTE. Nada foi criado na Meta ainda. Ao ser aprovado, o anuncio nasce ACTIVE (passa a poder entregar se campanha/conjunto estiverem ativos). Campanha e conjunto novos ainda nascem PAUSADOS. O pedido expira em 24h se nao for decidido."
      : "Pedido PENDENTE. Nada foi criado na Meta ainda. Ao ser aprovado por um administrador, campanha/conjunto nascem PAUSADOS (revisao no Gerenciador antes de ligar entrega). Anuncio novo (criar_anuncio) nasce ACTIVE. O pedido expira em 24h se nao for decidido." };
}
...
- Voce nao gasta nem publica por conta propria: toda acao real passa por card aprovado por humano, e as travas por acao sao dele. CONTRATO VIGENTE DESDE 15/08/2026: (1) aprovar criar_anuncio_a_partir_de CRIA o anuncio ACTIVE — a aprovacao do card JA e a autorizacao de entrega do criativo; (2) campanha e conjunto novos ainda nascem PAUSADOS (revisao estrutural no Gerenciador); (3) existe ativar_criativo para religar anuncio que ja estava PAUSED. Texto antigo dizendo que anuncio tambem nasce PAUSED ou que ativacao de criativo e so no Gerenciador esta VENCIDO. Voce continua sem caminho para gastar/pausar/ativar sem card aprovado. Trava fechada = explique o que falta e entregue o plano; nunca contorne.
...
pode ser inventada — ou, em peca nova, sem_molde=true + drive_file_id do acervo. Campanha e
conjunto novos nascem PAUSADOS (revisao no Gerenciador). Anuncio (criar_anuncio_a_partir_de)
nasce ACTIVE na aprovacao do card — nao peca segundo passo de ativacao no Gerenciador para o
criativo. Se um anuncio ja existente estiver PAUSED e o gestor pedir religar, use ativar_criativo.
