...
const TOOLS = [
  ...
  { type: "function", function: { name: "get_acervo_para_anuncio", description: "LEITURA TOTAL do acervo do Drive para MONTAR anuncio novo / escolher peca / inventariar midias. NAO use get_criativos_conteudo (so anuncios ja no ar). SEMPRE devolve taxonomia_drive + inventario_global: 19 videos (10 Educacao financeira + 9 Caminho Triste/feliz), Capas, 9 Carrosseis, 4 Cards instrucionais (leia a legenda). LEITURA livre: legivel=true; apta=true so = pronta pra publicar agora — NAO use apta para afirmar escassez. Em lote/mix: chame SEM produto primeiro e cite taxonomia_drive. Slides Carrossel = imagem estatica; formato carrossel Meta nao montado = limite de formato. Cards nao sao imagem generica. Capas inventariar. Bloqueadas: propor via veredito (nao liberar FIN-04 sozinho). incluir_inaptas default true.", parameters: { type: "object", properties: { produto: { type: "string", description: "Opcional. Em lote/mix deixe vazio na 1a chamada; so filtre depois se precisar estreitar." }, incluir_inaptas: { type: "boolean", description: "Padrao true (leitura total). false = so aptas (quase nunca use em inventario)." } } } } },
...
- MONTAR ANUNCIO NOVO / ESCOLHER PECA DO ACERVO / LOTE MIX: a fonte e get_acervo_para_anuncio
  ...
  autoriza dizer "so existem N"; (3) apta=false / bloqueada_por_compliance = ainda LEGIVEL:
  liste e, se o gestor quiser, emita registrar_veredito_peca_em_revisao (liberado_como_esta) +
  card de criacao apos aprovacao humana — NUNCA diga que a peca "nao existe"; NAO liberar
  FIN-04 sozinho; (4) slides Carrossel 1..9 = IMAGEM ESTATICA; formato carrossel Meta nao e
  montado = limite de FORMATO, nao ausencia; (5) Capas (Vídeos/Educação financeira/Capa) =
  inventariar sempre; (6) Cards = mecanismo instrucional "leia a legenda", nao imagem generica;
  (7) complemente com get_drive_criativos se precisar do inventario vivo. Se
  na_biblioteca_da_meta=false, upload_midia(drive_file_id). Video: so card com status ready.
...
pode ser inventada — ou, em peca nova, sem_molde=true + drive_file_id do acervo. Campanha,
