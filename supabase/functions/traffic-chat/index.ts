...
async function t_propose_action(companyId: string, convId: string, requestedBy: string, args: any, cards: CardInfo[]) {
  ...
  const VALID = ["pausar_criativo", "escalar_criativo", "pausar_campanha", "alterar_orcamento", "renomear_campanha"];
  ...
  const summary = ({ pausar_criativo: `Pausar o criativo "${alvo.name}"`, escalar_criativo: `Escalar o criativo "${alvo.name}"`,
    ...
    renomear_campanha: `Renomear a campanha "${alvo.name}" para "${String(params?.novo_nome ?? "").trim()}" via Pipeboard` } as Record<string, string>)[action];
...
async function t_renomear_campanha(companyId: string, convId: string, requestedBy: string, args: any, cards: CardInfo[]) {
  ...
  if (!novoNome) return { erro: "novo_nome obrigatorio" };
  ...
  return await t_propose_action(companyId, convId, requestedBy, {
    ...
    mecanismo: "Pipeboard update_campaign altera somente o campo name da campanha existente.",
    params: { novo_nome: novoNome },
  }, cards);
}
