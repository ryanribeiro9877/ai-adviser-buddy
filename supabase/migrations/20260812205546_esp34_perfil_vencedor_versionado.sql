...
--
-- Ate aqui o vencedor so existia como recomendacao efemera (evaluate_winners -> ai_recommendations,
-- apagada/reescrita a cada cron) ou como inferencia solta do agente. ESP-34 transforma o sinal de
-- vencedor num ARTEFATO versionado por empresa, com procedencia e lacunas declaradas, que pode ser
-- lido de volta ("qual era o perfil na versao X") sem depender do estado atual da fila.
--
-- Reaproveita a MESMA logica de evaluate_winners (ESP-01): janela, significancia (>=30 resultados e
-- >=30 de gasto), regua = teto_vigente() e limiar de vencedor custo <= teto*0,80. Enriquecimento de
-- procedencia (peca do Drive de origem) vem do mesmo vinculo de casar_criativo_performance (ESP-33):
-- approval_requests(criar_anuncio_a_partir_de) -> drive_file_id -> drive_midia_analises.
--
-- NAO publica nada e NAO substitui get_recommendations (fila acionavel) nem a aprovacao humana de
-- escala (ESP-39: vencedor mora em campanha ESCALA). E leitura + memoria versionada.
--
-- Aplicada como version 20260812205546. Espelho fiel em
-- supabase/espelhos/20260812205546_esp34_perfil_vencedor_versionado.sql

-- 1) Tabela versionada (uma linha = uma versao do perfil da empresa).
...
-- 2) Compute: calcula o perfil da empresa e grava nova versao (dedup no mesmo dia).
...
begin
  ...
  -- Padroes agregados a partir do proprio conjunto de vencedores.
...
-- 3) Read: le a ultima versao (ou uma especifica) com contagem de historico.
