-- [F5.5 fix] alerts.severity e enum alert_severity; a v1 passava text e quebrava no INSERT
-- (pego em prova com dado sintetico, nao em producao). Unica mudanca: cast ::alert_severity.
  ...
  loop
    if r.rank_atual is not null and r.rank_anterior is not null and r.rank_atual <> r.rank_anterior then
      ...
      if not exists (select 1 from alerts a where a.company_id = r.company_id
        ...
        values (r.company_id, v_sev::alert_severity, v_title, v_desc, false);
        criados := criados + 1;
      end if;
    end if;
