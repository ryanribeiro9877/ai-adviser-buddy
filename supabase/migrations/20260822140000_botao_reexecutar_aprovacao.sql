-- Botao "Pode tentar de novo" no ActionCard: dispara de novo a edge meta-actions
-- para um card JA aprovado cuja ultima tentativa falhou ANTES de qualquer escrita
-- (executed_at nulo, ultima_falha.re_executavel). O trigger so dispara na MUDANCA
-- para approved, entao retry precisa desta RPC. Admin only.
