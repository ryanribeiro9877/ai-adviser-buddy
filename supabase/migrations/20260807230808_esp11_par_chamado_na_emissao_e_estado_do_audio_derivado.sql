...
-- version: 20260807230808
-- name: esp11_par_chamado_na_emissao_e_estado_do_audio_derivado
-- projeto: gzjwnjdpxpbmdhcyefvs (Gestor de Trafego IA)
-- aplicada por: Claude via MCP apply_migration
-- espelho gerado a partir de supabase_migrations.schema_migrations - NAO transcrito a mao

-- CONSERTO 1: checar_par_texto_e_peca deixa de ser CITADO e passa a ser CHAMADO na emissao do
-- card. Quando ha os DOIS lados (legenda e peca com texto lido), um par que REPROVA recusa o
-- card (completo=false, recusa nomeada, mensagem que nomeia regra/numero/o que falta). Sem um
-- dos lados, declara a lacuna - nao aprova por omissao nem reprova por ausencia.
-- checar_par_texto_e_peca e STABLE, compativel com esta funcao STABLE.
--
-- CONSERTO 2: o texto sobre o audio ("so parte do acervo foi transcrita, e NENHUMA das pecas em
-- revisao esta entre as transcritas") virou mentira: os 19 videos estao transcritos. E "16 regras"
-- tambem envelheceu. Os dois passam a ser DERIVADOS do dado (contagem viva), para nao envelhecerem
-- de novo - o mesmo defeito que este projeto ja pagou com a nota da Fase 3 e o dry_run desatualizado.
...
BEGIN
  ...
  -- ==================== CONSERTO 1: o par legenda+peca CHAMADO, nao citado ====================
      ...
      IF (v_par->>'veredito') = 'reprova' THEN
        ...
        RETURN jsonb_build_object(
          ...
          'mensagem_para_o_gestor',
            ...
            || coalesce(nullif(v_par_seguro,''), 'inclua o CET, a taxa "a partir de" e a ressalva de analise de credito e margem.')
  ...
  -- ==================== CONSERTO 2: estado do audio e das regras DERIVADO do dado ====================
