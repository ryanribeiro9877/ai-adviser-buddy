...
--
-- Ate aqui o transporte do ultimo passo era UM driver por empresa (meta_execution_config.
-- driver_escrita, default graph; LEV em pipeboard). O unico caso por-acao era renomear_campanha,
-- que ja exigia pipeboard via if hardcoded. ESP-29 generaliza: cada acao pode ter seu proprio
-- driver, resolvido por resolver_driver(company_id, acao) com precedencia
--   override (driver_por_acao[acao]) > empresa (driver_escrita) > graph
-- e uma matriz de capacidade (renomear_campanha e pipeboard-only; demais acoes aceitam ambos).
-- pode_executar_acao e meta-actions passam a usar o driver RESOLVIDO. O driver decide ONDE o
-- ultimo passo sai, nunca SE sai (permissao continua em master_enabled + action_flags + limites).
--
-- Aplicada como version 20260812211515. Espelho fiel em
-- supabase/espelhos/20260812211515_esp29_driver_de_transporte_por_acao.sql
