...
-- outros dominios ficam FORA — nao inventamos URL. Normaliza http/https, www e trailing slash.
--
...
comment on function public.destino_url_lp_do_molde(uuid, text) is
  'Portao de emissao: le ads.destino_url (GT-12) do creative molde e devolve a resolucao canônica LP da Legal e Viver.';
...
-- Portao: apos completo+estado, anexa destino_url_lp. Nao recusa raiz do dominio LP —
...
begin
  ...
  if coalesce((v_dest->>'aplicavel')::boolean, false)
    ...
    v_msg := v_msg
      ...
      || '; na publicacao usarei o canônico '
...
comment on function public.pedido_de_anuncio_completo(uuid, jsonb) is
  'Gate antes do card. Mantem gates operacionais e de estado; para Legal e Viver anexa destino_url_lp (canônico /simulacao-clt) a partir do destino GT-12 do molde. Correcao da raiz do dominio LP e declarada, nao silenciosa.';
...
-- Contrato: documenta destino_url como campo opcional do pedido (a executora aplica o canônico).
...
select
  ...
  false,
  'text',
  'URL de destino do anuncio. Na Legal e Viver, LP/Site usa o canônico https://legaleviver.com.br/simulacao-clt: se o molde trouxer o mesmo dominio sem esse path, a executora CORRIGE (nao inventa URL para WhatsApp/outros dominios). Pedido e card podem carregar o valor ja resolvido para o gestor ver.',
...
values (
  'execucao',
  'DESTINO CANONICO LP/SITE (11/08/2026): anuncios de landing page / site da Legal e Viver usam SEMPRE https://legaleviver.com.br/simulacao-clt — nao a raiz legaleviver.com.br. Ao propor criar_anuncio_a_partir_de (peca nova ou replicacao), esse e o destino a declarar. Se o molde trouxer http/https, www ou path vazio/outro no mesmo dominio, o sistema CORRIGE para o canônico na emissao (pedido_de_anuncio_completo.destino_url_lp) e na executora; WhatsApp (wa.me etc.) nao e reescrito. Nao invente URL para produto que nao seja LP/Site.',
  true,
  current_date,
