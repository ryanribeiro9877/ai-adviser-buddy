-- Click-to-WhatsApp: "IN_ACTIVE_ADS" exige entrega efetiva.
-- Antes: ad ACTIVE OU campanha active marcava o número como em campanha ativa
-- (ex.: ad ACTIVE com adset PAUSED / campanha só "active" no espelho stale).
-- Agora: campanha active + adset ACTIVE + ad ACTIVE (caminho ads);
--        campanha active + adset ACTIVE (caminho WA- no nome do conjunto).
...
begin
    ...
    for v_digits, v_rotulo, v_campanhas, v_conjuntos, v_ativos in
      with urls as (
        select
          ...
          s.name as conjunto,
          case
            when lower(coalesce(c.status, '')) in ('active', 'ativo')
    ...
    loop
      ...
      insert into public.waba_phone_numbers as p (
        ...
        jsonb_build_object(
          ...
          'criterio_ativo', 'campaign+adset+ad ACTIVE',
  ...
  return jsonb_build_object(
    ...
    'versao', 'whatsapp-ads-destino-v2-entrega-efetiva'
  );
end;
$$;
