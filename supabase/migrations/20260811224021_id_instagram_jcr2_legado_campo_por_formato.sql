-- Ryan informou em 11/08/2026 o id da conta Instagram oficial @jcr2_legaleviver, lido na tela
-- Identidade do Gerenciador de Anuncios: 1296945687078272.
--
-- Por que este id e diferente em NATUREZA dos anteriores:
--   - GET /1296945687078272 na Graph (sonda meta-identity-probe, request 630) responde erro 36106
--     "This legacy Instagram endpoint/object is deprecated" - ou seja, o objeto EXISTE e e um ator
--     Instagram LEGADO; o endpoint de leitura e que foi descontinuado.
--   - O id anterior 17841428674060566 respondia 100/subcode 33 "does not exist" - objeto inexistente.
--   Erro de endpoint descontinuado != erro de objeto inexistente. Por isso este id e aceito e
--   aquele nao era.
--
-- O que NAO foi possivel confirmar (registrado como ressalva, nao como fato):
--   - O vinculo com a pagina 1095196357012756 nao e verificavel: o token nao tem
--     pages_read_engagement (escopos: ads_management, ads_read, business_management, whatsapp_*,
--     public_profile) e owned_instagram_accounts do Business volta vazio. Isso e LIMITE DE
--     PERMISSAO, nao prova de ausencia. instagram_identity_page_link_confirmed segue false.
--   - O username tambem nao foi confirmado pela Graph pelo mesmo motivo. O handle gravado e o que
--     o Ryan leu no Gerenciador.
--
-- Consequencia de CAMPO (o ponto central): 1296945687078272 tem formato de instagram_actor_id
-- LEGADO, nao de instagram_user_id (que e o id de Instagram Business Account, formato 1784 + 13
-- digitos). meta-actions v5.16 passa a escolher o campo do object_story_spec pelo FORMATO do id.
--
-- Aplicada como version 20260811224021. Espelho fiel em
-- supabase/espelhos/20260811224021_id_instagram_jcr2_legado_campo_por_formato.sql
...
-- Contrato: declarar o campo do spec como derivado do formato do id, com procedencia nas linhas.
...
-- Doutrina: supersede a versao anterior (que dizia id NULL / nao comprovado)
