...
-- version: 20260723124306
-- name: f22_evaluate_winners
-- projeto: gzjwnjdpxpbmdhcyefvs (Gestor de Trafego IA)
-- aplicada por: Claude via MCP apply_migration
-- espelho gerado a partir de supabase_migrations.schema_migrations - NAO transcrito a mao

-- F2.2 — Detector de criativos vencedores ("produza mais desse tipo", call do Roberto).
-- PARÂMETROS (documentados; ajustar aqui se o Roberto pedir):
--   Janela: 7 dias · Significância mínima: >=30 resultados E >=R$30 gastos na janela
--   Vencedor: custo/resultado da janela <= 80% do teto vivo (targets, mesma métrica da R5:
--             mensagem->custo_por_conversa; leadgen->custo_por_formulario, fallback lead_lp)
-- SAÍDAS em ai_recommendations (aba de recomendações do front lê direto):
--   (a) category='escala'   impact='high'   -> escalar orçamento/duplicar o vencedor
--   (b) category='criativo' impact='medium' -> produzir mais criativos desse tipo (atributos)
-- Idempotente: apaga e regrava só as pendentes marcadas [auto: vencedores];
-- recomendações já tratadas (status <> 'pendente') são preservadas como histórico.
-- Hardening de grants no padrão sec_revoke (cron roda como dono; sem acesso anon).
...
begin
  delete from public.ai_recommendations
   where status = 'pendente' and description like '%[auto: vencedores]%';
  ...
  with base as (
    select s.ad_external_id,
           ...
           sum(case when max_cat.cat = 'mensagem' then s.messaging_started else s.form_leads end) as results7
    from public.ad_metric_snapshots s
    join public.ads a on a.external_id = s.ad_external_id
    join public.campaigns c on c.id = a.campaign_id
    cross join lateral (select c.category as cat) max_cat
  ...
  -- (a) escalar + (b) produzir similar, numa passada
