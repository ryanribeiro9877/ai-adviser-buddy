...
-- version: 20260804132623
-- name: relatorio_diario_com_quebra_por_campanha
-- projeto: gzjwnjdpxpbmdhcyefvs (Gestor de Trafego IA)
-- aplicada por: Claude via MCP apply_migration
-- espelho gerado a partir de supabase_migrations.schema_migrations - NAO transcrito a mao

-- 04/08/2026 - RELATORIO DIARIO PASSA A QUEBRAR POR CAMPANHA (pedido do Ryan, 04/08).
-- DEFEITO QUE ISSO CORRIGE: o relatorio somava tudo no nivel da EMPRESA - "ontem: gasto R$ X,
-- N leads, N formularios" - com 21 campanhas na conta. Metrica agregada mente por construcao:
-- o numero nao dizia nada sobre nenhuma campanha. E impressoes e alcance, que ja eram
-- coletados, nunca apareciam em lugar nenhum do relatorio.
-- ESCOLHA DECLARADA: cobre toda campanha com gasto no dia, MAIS as que tem teto declarado
-- mesmo sem gasto - campanha que parou de gastar sem aviso e informacao, nao silencio.
-- O QUE AINDA NAO EXISTE e por isso NAO aparece: recorte por publico/demografia e os tres
-- rankings de qualidade da Meta (quality_ranking, engagement_rate_ranking,
-- conversion_rate_ranking). Nenhum dos dois e coletado por nenhuma rotina. O relatorio
-- DECLARA essa ausencia em vez de omitir - ausencia silenciosa e a pior forma de lacuna.
...
begin
  ...
  loop
    ...
    if conv_id is null then
      ...
      values (emp.id, 'Relatório diário', 'daily_report')
    ...
    -- ===== totais da empresa (mantidos: servem de fechamento, nunca de retrato) =====
    ...
    with base as (
      select c.id, c.name,
        ...
        left join public.metric_snapshots m
               on m.campaign_id = c.id and m.snapshot_date = v_d1
    ...
    select count(*), string_agg(
      '### ' || name || e'\n'
      || '- Gasto **R$ ' || to_char(sp,'FM999G999D00') || '**'
         || case
              when teto is null then ' · sem teto declarado'
              when sp = 0 then ' · teto declarado R$ ' || to_char(teto,'FM999G999D00') || ' — **sem gasto ontem**'
              when sp > teto then ' · teto declarado R$ ' || to_char(teto,'FM999G999D00')
                   || ' → **' || to_char(round(100*sp/teto),'FM999') || '% do teto** ⚠️'
              else ' · teto declarado R$ ' || to_char(teto,'FM999G999D00')
                   || ' → ' || to_char(round(100*sp/teto),'FM999') || '% do teto ✅'
            end
         || case when media7 is not null and media7 > 0 and sp > 0 then
              ' · vs média 7d (R$ ' || to_char(media7,'FM999G999D00') || '): '
              || case when sp > media7 then '+' else '' end
              || to_char(round(100*(sp-media7)/media7),'FM999') || '%'
            else '' end || e'\n'
      || case when sp = 0 then ''
         else
           '- **' || to_char(imp,'FM999G999') || '** impressões para **' || to_char(rch,'FM999G999')
           || '** pessoas' || case when freq is not null then ' (frequência ' || to_char(freq,'FM990D00') || ')' else '' end || e'\n'
           || '- **' || clk || '** cliques · **' || lclk || '** no link · **' || lpv || '** chegaram na página' || e'\n'
           || '- **' || frm || '** formulários'
           || case when frm > 0 then
                ' · custo/formulário **R$ ' || to_char(round(sp/frm,2),'FM990D00') || '**'
                || case when v_teto_form is not null then
                     case when round(sp/frm,2) <= v_teto_form
                          then ' (dentro do teto R$ ' || to_char(v_teto_form,'FM990D00') || ' ✅)'
                          else ' (**ACIMA** do teto R$ ' || to_char(v_teto_form,'FM990D00') || ' ⚠️)' end
                   else '' end
              else ' — nenhum formulário' end || e'\n'
         end,
      e'\n' order by sp desc, name)
      into v_n_camp, v_campanhas
      from enriquecida;
    ...
    if exists (select 1 from public.wabas w where w.company_id = emp.id) then
      ...
      select '**' || coalesce(nullif(t.template_name,''), w.name, t.template_external_id) ||
             '** (' || coalesce(sum(t.clicked),0) || ' cliques · ' || coalesce(sum(t.sent),0) || ' envios em 7d)'
        into v_top_template
      ...
      v_waba :=
        ...
        '- Ontem: **' || coalesce(v_sent_ontem,0)::bigint || '** mensagens enviadas (agregado das contas)' ||
    ...
    corpo :=
      ...
      '**Fechamento da empresa:** gasto **R$ ' || to_char(v_gasto,'FM999G999D00') || '** · ' ||
      v_links || ' cliques no link · ' || v_forms || ' formulários' ||
      case when v_custo_form is not null then ' · custo/formulário médio **R$ ' || to_char(v_custo_form,'FM990D00') || '**' ||
        case when v_teto_form is not null then
