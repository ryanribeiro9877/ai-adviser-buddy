...
-- version: 20260806214842
-- name: alinhar_nomes_pedido_contrato_executor
-- projeto: gzjwnjdpxpbmdhcyefvs (Gestor de Trafego IA)
-- aplicada por: Claude via MCP apply_migration
-- espelho gerado a partir de supabase_migrations.schema_migrations - NAO transcrito a mao

-- Alinha pedido_de_anuncio_completo ao nome que montarCriacao / meta-actions consome.
...
(
  ...
  false,
  'text',
  'procedencia da legenda (humano|herdada_do_molde|agente). montarCriacao nao manda a Graph; meta-actions grava em ads.legenda_fonte no espelho. Em peca_nova, pedido_de_anuncio_completo EXIGE antes do card.',
  'supabase/functions/meta-actions/index.ts · espelhar · L751; pedido_de_anuncio_completo le p_pedido->>legenda_fonte'
)
...
comment on function public.pedido_de_anuncio_completo(uuid, jsonb) is
  'Verifica se o pedido de anuncio esta completo ANTES de emitir card. Nome canonico do conjunto: conjunto_destino_external_id (mesmo que montarCriacao). Alias legado conjunto_destino ainda aceito. legenda_fonte obrigatoria em peca_nova.';