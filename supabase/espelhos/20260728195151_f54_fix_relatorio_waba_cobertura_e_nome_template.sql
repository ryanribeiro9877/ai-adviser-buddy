-- [F5.4 fix, pego em prova] Dois defeitos de dado que a secao WhatsApp herdaria:
-- (1) waba_analytics_daily tem linhas com phone_external_id = '' (agregado da WABA, nao por
--     numero). Contar '' como "1 numero coberto" seria mentira - agora nullif() exclui e a
--     linha declara honestamente 0 de 11 ate a coleta por numero existir.
-- (2) linhas com clique tem template_name NULL; o nome real vem do join com waba_templates.
-- Unicas mudancas vs versao anterior: nullif() nos dois pontos e o join do template.
...
begin
  ...
  loop
    ...
    if exists (select 1 from public.wabas w where w.company_id = emp.id) then
      ...
      -- nullif('') : linha agregada da WABA sem numero NAO conta como "numero coberto".
      ...
      select '**' || coalesce(nullif(t.template_name,''), w.name, t.template_external_id) ||
             '** (' || coalesce(sum(t.clicked),0) || ' cliques · ' ||
             coalesce(sum(t.sent),0) || ' envios em 7d)'
        into v_top_template
       ...
       having coalesce(sum(t.clicked),0) > 0
       order by sum(t.clicked) desc
       limit 1;
      ...
      v_waba :=
        ...
        case when coalesce(v_n_numeros,0) > 0 then
          ...
          ' números (sem dado por número NÃO significa zero envio)'
        else '' end || e'\n' ||
