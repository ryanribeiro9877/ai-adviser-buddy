...
| `20260730175943_fix_get_criativos_conteudo_isolamento_empresa.sql` | 30/07 | **Vazamento entre empresas:** `get_criativos_conteudo` varria `ads` sem filtro de `company_id` (peça da COHAPM entrava no portfólio de crédito da Legal). Nova assinatura com `p_company_id` obrigatório + legada mantida com aviso e campo `empresa` por item. ⚠️ **A edge ainda chama a legada** — migrar e aposentar. |
