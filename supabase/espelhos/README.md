...
| `20260903200709_camada_de_conhecimento_deterministico_seed.sql` | Camada determinística | Os 14 moldes. Motivo medido: as 13 `perguntas_ouro` rodaram 40 vezes pelo caminho de geração livre e saíram **40 respostas distintas** — zero repetição byte a byte, e em 12 dos 13 códigos o conjunto de números citados mudou entre rodadas (PO-01 rodou 3x no mesmo dia contra a mesma RPC: duas citaram R$ 1.512,00 como pior dia, a terceira não citou esse valor em lugar nenhum). Os 6 textos canônicos são duplicados byte a byte no fallback de `_shared/resposta_canonica.ts` e a duplicação é conferida por hash sha256 na prova. |
