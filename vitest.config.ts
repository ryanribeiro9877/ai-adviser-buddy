...
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      ...
      // Só o código que os testes têm por objetivo cobrir. Sem esta lista, os 21
      // componentes de ui/ (wrappers de Radix, sem lógica própria) e o
      // routeTree.gen.ts diluiriam o número e o portão de cobertura mediria
      // ruído em vez de risco.
      include: ["src/lib/**/*.ts", "src/hooks/**/*.ts"],
      exclude: ["src/lib/error-capture.ts", "src/lib/error-page.ts"],
      // Piso deliberadamente baixo: é o que a suíte cobre HOJE, e serve de
      // catraca contra regressão. Subir junto com a cobertura, nunca antes.
      thresholds: { lines: 55, functions: 55, statements: 55, branches: 70 },
    },
  },
});
